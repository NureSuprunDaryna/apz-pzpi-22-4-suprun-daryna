...
namespace Server.Controllers
{
    ...
    public class ElixirController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateElixir([FromBody] CreateElixirModel model)
        {
            ...
            elixir.MapFrom(model);
            elixir.AuthorId = userId;
            elixir.Author = user;
        ...
        public async Task<IActionResult> EditElixir([FromBody] UpdateElixirModel model)
        {
            ...
            var result = await _elixirService.UpdateElixir(existingElixir.Data);
        ...
        [HttpGet("{elixirId}")]
        public async Task<IActionResult> GetElixirById(int elixirId)
        {
            if (elixirId <= 0)
            {
                _logger.LogWarning("Attempted to retrieve elixir with invalid ID: {ElixirId}", elixirId);
                return BadRequest("Elixir ID must be greater than zero.");
            }

            _logger.LogInformation("Request received to retrieve elixir with ID {ElixirId}", elixirId);

            var result = await _elixirService.GetById(elixirId);
            ...
            var response = new ElixirDetailedResponseModel
            {
                Id = result.Data.Id,
                Name = result.Data.Name,
                Description = result.Data.Description,
                Compositions = result.Data.ElixirComposition.Select(comp => new ElixirCompositionResponseModel
                {
                    NoteCategory = comp.NoteCategory,
                    Proportion = comp.Proportion,
                    Note = new NoteResponseModel
                    {
                        Id = comp.Note.Id,
                        Name = comp.Note.Name
                    }
            ...
            _logger.LogInformation("Successfully retrieved elixir with ID {ElixirId}", elixirId);
        ...
        {
            _logger.LogInformation("Request received to retrieve elixirs for author with ID {AuthorId}", authorId);

            if (string.IsNullOrWhiteSpace(authorId))
            {
                _logger.LogWarning("Invalid request: authorId is null or empty.");
                return BadRequest("Author ID cannot be null or empty.");
            }

            var elixirsResult = await _elixirService.GetElixirsByAuthor(authorId);

            if (!elixirsResult.IsSuccessful)
            {
                _logger.LogError("Failed to retrieve elixirs for author {AuthorId}. Error: {Error}", authorId, !elixirsResult.IsSuccessful);
                return StatusCode(StatusCodes.Status500InternalServerError, "Failed to retrieve elixirs.");
            }

            var response = elixirsResult.Data.Select(elixir => new ElixirDetailedResponseModel
            {
                Id = elixir.Id,
                Name = elixir.Name,
                Description = elixir.Description,
                Compositions = elixir.ElixirComposition.Select(comp => new ElixirCompositionResponseModel
                {
                    NoteCategory = comp.NoteCategory,
                    Proportion = comp.Proportion,
                    Note = new NoteResponseModel
            ...
            _logger.LogInformation("Successfully retrieved {ElixirCount} elixirs for author {AuthorId}", response.Count, authorId);
